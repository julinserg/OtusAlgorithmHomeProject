# file: features/minisearch.feature

# http://localhost:8088/
# http://minisearch_service:8088/

Feature: Тесты превьюера изображений с проверкой работы кеша

	Scenario: Доступность сервиса превьюера
		When I send "GET" request to "http://minisearch_service:8088/"
		Then The response code should be 200
		And The response should match text "This is my minisearch!"

	Scenario: Очистка кеша сервиса превьюера
		When I send "GET" request to "http://minisearch_service:8088/clearcache"
		Then The response code should be 200
	...
	Scenario: Получение превью изображения gopher_50x50.jpg с удаленного сервера
		When I send "GET" request to "http://minisearch_service:8088/fill/50/50/web/images/gopher_original_1024x504.jpg"
		Then The response code should be 200
	...
	Scenario: Получение превью изображения gopher_200x700.jpg с удаленного сервера
		When I send "GET" request to "http://minisearch_service:8088/fill/200/700/web/images/gopher_original_1024x504.jpg"
		Then The response code should be 200
